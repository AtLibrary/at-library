...
public class WebActionSteps {
    ...
    /**
     * ---------------------------------------Нажатие (ховер) на элемент в блоке----------------------------------------
     */

    //TODO

    /**
     * ---------------------------------------Установка значения в поле в блоке-----------------------------------------
     */
     ...
     * @return
     */
    @То("^в поле \"([^\"]*)\" в блоке \"([^\"]*)\" введено значение$")
    @И("^в поле \"([^\"]*)\" в блоке \"([^\"]*)\" введено значение \"([^\"]*)\"$")
    public String setBlockFieldValue(String elementName, String blockName, String value) {
        value = getPropertyOrStringVariableOrValue(value);
        SelenideElement valueInput = coreScenario.getCurrentPage().getBlock(blockName).getElement(elementName);
        valueInput.setValue(value);
        return value;
    }
