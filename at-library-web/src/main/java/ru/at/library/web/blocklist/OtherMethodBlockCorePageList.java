...
import static ru.at.library.web.BrowserSteps.takeScreenshot;
 ...
 * -----------------------------------------------------------------------------------------------------------------
 */
public class OtherMethodBlockCorePageList {
    ...
    public static List<CorePage> getBlockListWithCheckingTheQuantity(String listName, CustomCondition.Comparison comparison, int count) {
        ...
        List<CorePage> blocksList = CoreScenario.getInstance().getCurrentPage().getBlocksList(listName);
    ...
    @Step("Поиск блока соответсвующего условиям")
    public static List<CorePage> getBlockListWithComplexCondition(String blockListName, DataTable conditionsTable) {
        ...
        String resultMessageTemplate = "Найден блок(и) где у элемента %s текст %s %s :\n%s\n";
        StringBuilder resultMessage = new StringBuilder();

        try {
            List<CorePage> blockList = CoreScenario.getInstance().getCurrentPage().getBlocksList(blockListName);
            for (List<String> conditionsRow : conditionsRows) {
                ...
                String expectedValue = getPropertyOrStringVariableOrValue(conditionsRow.get(2));
    ...
    private static List<CorePage> findCorePageByConditionInElement(List<CorePage> blockList, String elementName, String elementCondition, String expectedValue) {
        ...
        if (resultList.isEmpty()) {
            throw new AssertionError("В списке блоков не найден элемент " + elementName
                    + " текст которого " + elementCondition + " " + expectedValue
